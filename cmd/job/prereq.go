...
func ensureGCloudAuthenticated() error {
	...
	if result.Err != nil && (errors.Is(result.Err, context.DeadlineExceeded) || result.Err.Error() == "signal: killed") {
...
func ensureProjectExists(projectID string) error {
	...
	if result.Err != nil && (errors.Is(result.Err, context.DeadlineExceeded) || result.Err.Error() == "signal: killed") {
		return fmt.Errorf("gcloud projects describe timed out after %v for project %q. Please check your network connection", timeoutDuration, projectID)
	}
...
func checkArtifactRegistryAPI(projectID string, state *PrereqState, missing *[]missingPrereq) {
	...
	if apiResult.Err != nil && (errors.Is(apiResult.Err, context.DeadlineExceeded) || apiResult.Err.Error() == "signal: killed") {
